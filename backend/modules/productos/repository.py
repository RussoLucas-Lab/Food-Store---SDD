...
class InMemoryProductRepository(IProductRepository):
    ...
    def __init__(self):
        ...
        self._category_index: Dict[int, set] = {}
        self._ingredient_index: Dict[int, set] = {}
    ...
    def find_by_category(self, category_id: int) -> List[Product]:
        """Buscar productos de una categoría (mock)"""
        return []
    ...
    def restore_stock(self, producto_id: int, cantidad: int) -> bool:
        """
        Restaurar (incrementar) el stock de un producto tras cancelación de pedido.

        Se usa cuando se cancela un pedido en estado CONFIRMADO o EN_PREPARACION,
        en los que el stock ya fue decrementado. Devuelve False si el producto
        no existe en el repositorio.

        Args:
            producto_id: ID del producto a restaurar
            cantidad: Unidades a devolver al stock

        Returns:
            True si se restauró, False si el producto no existe
        """
        product = self._storage.get(producto_id)
        if not product:
            return False
        # El campo `stock` es el que usa el UoW in-memory durante validación de pedidos.
        current_stock = getattr(product, "stock", 0)
        product.stock = current_stock + cantidad
        return True
...
class InMemoryProductIngredientRepository(IProductIngredientRepository):
    ...
    def __init__(self):
        ...
        self._next_id = 1
        self._product_index: Dict[int, List[int]] = {}
        self._ingredient_index: Dict[int, List[int]] = {}
